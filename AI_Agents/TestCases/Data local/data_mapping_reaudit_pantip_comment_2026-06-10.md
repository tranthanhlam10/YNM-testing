...
Mapping hiện tại **chưa pass**. Có ba lỗi chắc chắn ảnh hưởng trực tiếp đến các field BA yêu cầu:
...
### Field ngoài scope mapping của BA

Output có thêm `title` và `updated_at`; API cũng có dữ liệu để tính `comments`, `likes` và `reaction`. Tuy nhiên bảng **Crawl comment** của BA không yêu cầu map các field này, vì vậy:

- Không đánh giá `title = ""` là lỗi.
- Không đánh giá thiếu `comments`, `likes`, `reaction` là lỗi.
- Không đánh giá giá trị `updated_at` là lỗi mapping trong phạm vi testcase hiện tại.
- Nếu cần validate hoặc yêu cầu dev map các field trên, BA phải bổ sung contract trước.
...
4. Phân biệt `img-in-emotion`/sticker với ảnh nội dung trước khi tạo attachment.
5. Xác nhận contract cho `id_reference`, `id_parent_comment` và photo `href`.
6. Deduplicate Identity theo `id` trước khi publish.
